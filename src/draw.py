...
# Define the function that will run the video player inside the GUI
def play():
    ...
    while video.isOpened():
        ...
        # Create a blank frame to be used as the mouse cursor
        cursor = np.zeros((10, 10, 3), dtype=np.uint8)

        # Draw the cursor on the screen in the current position of the mouse
        cv2.imshow("Cursor", cursor)
        cv2.moveWindow("Cursor", mouse_pos[0], mouse_pos[1])
...
window.title("Draw on Video")
# Set the window size
window.geometry('320x320')

# Create a button for starting the video
btn = Button(window, text="Play Video", bg="black", fg="white", font=("Arial", 10), command=play)
btn.place(x=80, y=80)
